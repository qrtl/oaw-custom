...
class AccountInvoiceLine(models.Model):
    ...
    number = fields.Char(
        related="invoice_id.number", store=True, string="Number"
    )
    state = fields.Selection(
        related="invoice_id.state", store=True, string="Status"
    )
    ...
    date_due = fields.Date(
        related="invoice_id.date_due", string="Due Date"
    )
    currency_id = fields.Many2one(
        related="invoice_id.currency_id", string="Currency"
    )
    partner_ref = fields.Char(
        "Supplier Reference", related="po_id.partner_ref",
    )
